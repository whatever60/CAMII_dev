...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Program of colony contour detection and segmentation. See details in https://github.com/hym0405/CAMII"
    )
    ...
    for i in range(total_image):
        tmpThread = threading.Thread(
            target=ipf.multi_fun0_detectColonySingleImage,
            args=(
                image_trans_list[i],
                image_epi_list[i],
                image_label_list[i],
    ...
    for i in range(total_image):
        tmpFlag = ipf.fun1_runPlateQualityControl(
            globalOutput.image_trans_corrected[i],
            globalOutput.image_epi_corrected[i],
            globalOutput.all_contours[i],
    ...
    ipf.saveOutputs_colonyDetection(
        globalOutput, total_image, configure_pool, output_dir
    )
