...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Program of morphology-guided colony selection for isolation. See details in https://github.com/hym0405/CAMII"
    )
    ...
    for eachGroupID in unique_groupID:
        ...
        ipf.fun3_runColonyQualityControl_group(
            eachGroupID, groupID_index, globalOutput, configure_pool, sample_config
        )
    ...
    for eachGroupID in unique_groupID:
        groupID_index = ipf.findGroupIDindex(globalOutput, eachGroupID)
        ipf.saveOutputs_pickingOptimization(
            globalOutput,
            len(globalOutput.image_label),
            configure_pool,
            output_dir,
            groupID_index,
        )
    ...
    ipf.savePCAdata(
        ...
        sample_config_path,
        output_dir,
        output_dir + "/PCAdata.csv",
    )

    os.system(
        "Rscript "
        + configure_pool["house_bin"]
        + "/visualizePCAandPickStatus.R "
        + output_dir
        + "/PCAdata.csv "
        + output_dir
    )
