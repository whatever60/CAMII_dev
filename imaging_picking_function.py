...
class globalOutputObject(object):
    def __init__(self, total_image):
        ...
        self.image_trans_corrected = [
            " ",
        ] * total_image
        self.image_epi_corrected = [
            " ",
        ] * total_image
        self.final_pick = [
            " ",
...
def multi_fun0_detectColonySingleImage(
    image_trans_path, image_epi_path, image_label, configure_pool, varPool, index
):
    (
        image_trans_corrected,
        image_epi_corrected,
        all_contours,
        all_metadata,
    ) = fun0_detectColonySingleImage(
        image_trans_path, image_epi_path, image_label, configure_pool
    )
...
def fun0_detectColonySingleImage(
    image_trans_path, image_epi_path, image_label, configure_pool
):
    ...
    print(
        "Finish "
        + image_label
        + " image loading... (Execution time: "
        + str(time_dur)
        + ")"
    )

    # crop the images
    image_trans_crop = crop_image(
        image_trans_raw, cropXMin, cropXMax, cropYMin, cropYMax
    )
    ...
    # correct image
    image_trans_corrected = (
        image_trans_crop.astype(np.float32) / image_trans_calib.astype(np.float32)
    ) * calib_contrast_trans_alpha + calib_contrast_trans_beta
    image_epi_corrected = image_epi_crop.astype(np.float32)
    image_epi_corrected[:, :, 0] = image_epi_corrected[
        :, :, 0
    ] / image_epi_calib_B.astype(np.float32)
    image_epi_corrected[:, :, 1] = image_epi_corrected[
        :, :, 1
    ] / image_epi_calib_G.astype(np.float32)
    image_epi_corrected[:, :, 2] = image_epi_corrected[
        :, :, 2
    ] / image_epi_calib_R.astype(np.float32)
    ...
    image_mask_bg = cv2.adaptiveThreshold(
        image_gray_first_con.astype(np.uint8),
        255,
        cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
        cv2.THRESH_BINARY_INV,
        bg_threshold_blockSize,
        bg_threshold_offset,
    )

    ##	image_mask_bg = cv2.inRange(image_gray_first, 0, bg_gray_mean - background_STD * bg_gray_sd)
    image_res_bg = cv2.bitwise_and(
        image_gray_first, image_gray_first, mask=image_mask_bg
    )
    time_dur = round(time.time() - start_time, 2)
    start_time = time.time()
    print(
        "Finish "
        + image_label
        + " background removal... (Execution time: "
        + str(time_dur)
        + ")"
    )
    ...
    print(
        "Finish "
        + image_label
        + " first run of colony detection... (Execution time: "
        + str(time_dur)
        + ")"
    )

    finalContours, finalDF = filterContours(
        contours,
        configure_pool,
        image_trans_corrected,
        image_epi_corrected,
    ...
    print(
        "Finish "
        + image_label
        + " colony filtering... (Execution time: "
        + str(time_dur)
        + ")"
    )

    post_finalContours = postprocess_contours(
        finalDF, finalContours, image_trans_corrected, configure_pool
    )
    time_dur = round(time.time() - start_time, 2)
    start_time = time.time()
    print(
        "Finish "
        + image_label
        + " multi-colony segmentation... (Execution time: "
        + str(time_dur)
        + ")"
    )

    post_finalContours_final = filterContours_final(
        post_finalContours,
        configure_pool,
        image_trans_crop,
        image_epi_crop,
    ...
    post_finalDF = getFinalData(
        post_finalContours_final,
        configure_pool,
        image_trans_corrected,
        image_epi_corrected,
        height_crop,
    ...
    print(
        "Finish "
        + image_label
        + " final data gathering... (Execution time: "
        + str(time_dur)
        + ")"
    )
    return (
...
def fun1_runPlateQualityControl(
    image_trans_crop,
    image_epi_crop,
    post_finalContours,
    post_finalDF,
    ...
    start_time = time.time()
    image_all_contours = drawContour(
        image_trans_crop, post_finalContours, plateQC_colonyContourPixel
    )
    image_all_contours_all_pin = drawPinSite(
        image_all_contours, post_finalContours, plateQC_colonyPinSize
    )
    flag = plateQualityControl(
        image_all_contours_all_pin,
        plateQC_imageScaleFactor,
        (
            width_crop * plateQC_imageScaleFactor + plateQC_imageWidthBias,
            height_crop * plateQC_imageScaleFactor + plateQC_imageHeightBias,
        ),
        plateQC_ComfirmWindow,
        image_label,
        len(post_finalContours),
        plateQC_TextSizeLarge,
        plateQC_TextSizeSmall,
        plateQC_TextSizeButton,
    )
    time_dur = round(time.time() - start_time, 2)
    print(
        "Finish " + image_label + " plate QC... (Execution time: " + str(time_dur) + ")"
    )
    return flag
...
def fun2_pickColonyPilot(post_finalDF, num_of_colonies, image_label, configure_pool):
    farthest_points_iteration = configure_pool["farthest_points_iteration"]
    start_time = time.time()
    pick_choice, post_finalDF_PCA = pickColonyFirst(
        post_finalDF, num_of_colonies, farthest_points_iteration
    )
    time_dur = round(time.time() - start_time, 2)
    print(
        "Finish "
        + image_label
        + " first run of colony selection... (Execution time: "
        + str(time_dur)
        + ")"
    )
...
def fun3_runColonyQualityControl_group(
    eachGroupID, groupID_index, varPool, configure_pool, sample_config
):
    ...
    if len(groupID_index) > 0:
        ...
        groupID_image_trans_list = [
            varPool.image_trans_corrected[i] for i in groupID_index
        ]
        ...
        groupID_totalColonies = sum(
            [len(varPool.all_contours[i]) for i in groupID_index]
        )
        groupID_colony_to_pick = getNumPickColonies(sample_config, eachGroupID)

        groupID_image_merge, groupID_image_heightStart = concatenateImages_gray(
            groupID_image_trans_list,
            groupID_label_list,
            spacing,
            fontSize,
        ...
        groupID_contour_merge = mergeModifyContour(
            groupID_contour_list, groupID_image_heightStart
        )
        groupID_metadata_merge = concat_metadata(
            groupID_metadata_list, groupID_image_heightStart
        )
        height_crop, width_crop = groupID_image_trans_list[0].shape
        groupID_init_pickChoice, groupID_metadata_merge_PCA = fun2_pickColonyPilot(
            groupID_metadata_merge, groupID_colony_to_pick, eachGroupID, configure_pool
        )
        final_pick, bad_pick = fun3_runColonyQualityControl(
            height_crop,
            width_crop,
            groupID_image_merge,
            groupID_contour_merge,
            groupID_metadata_merge,
            groupID_init_pickChoice,
            groupID_metadata_merge_PCA,
            eachGroupID,
            configure_pool,
        )
        tmp_pickStatus = [
            "not_pick",
        ] * groupID_metadata_merge.shape[0]
        for e in final_pick:
        ...
        groupID_metadata_splitIndex = getMetadataLabelIndex(
            groupID_metadata_merge, groupID_label_list
        )
        for i in range(len(groupID_index)):
            image_index = groupID_index[i]
            varPool.all_metadata[image_index] = groupID_metadata_merge.iloc[
                groupID_metadata_splitIndex[i]
            ]
            varPool.all_metadata_PCA[image_index] = groupID_metadata_merge_PCA[
                groupID_metadata_splitIndex[i]
            ]
            tmpMetadata, tmp_finalPick, tmp_badPick = modifyMetadataSplit(
                varPool.all_metadata[image_index]
            )
...
def fun3_runColonyQualityControl(
    height_crop,
    width_crop,
    image_trans_crop,
    post_finalContours,
    ...
    start_time = time.time()
    final_pick, bad_pick = colonyQualityControl(
        height_crop,
        width_crop,
        image_trans_crop,
        colonyQC_imageScaleFactor,
        (
            width_crop * colonyQC_imageScaleFactor + colonyQC_imageWidthBias,
            height_crop * colonyQC_imageScaleFactor + colonyQC_imageHeightBias,
        ),
        colonyQC_ComfirmWindow,
        colonyQC_colonyShowBias,
        (width_crop * colonyQC_imageScaleFactor - colonyQC_colonyWindowBias)
        / colonyQC_colonyColumnNum,
        image_label,
        post_finalContours,
        pick_choice,
        post_finalDF,
        post_finalDF_PCA,
        colonyQC_colonyContourPixel,
        colonyQC_colonyLabelSize,
        colonyQC_colonyLabelThickness,
        colonyQC_TextSizeLarge,
        colonyQC_TextSizeMid,
        colonyQC_TextSizeSmall,
        colonyQC_TextSizeButton,
    )
    time_dur = round(time.time() - start_time, 2)
    print(
        "Finish "
        + image_label
        + " colony QC... (Execution time: "
        + str(time_dur)
        + ")"
    )
...
def concatenateImages_gray(image_list, label_list, spacing, fontSize, fontThickness):
    ...
    for i in range(len(image_list)):
        ...
        cv2.putText(
            blankImage,
            eachLabel,
            (fontSize * 10, spacing - fontSize * 10),
...
def calculate_calib_image(trans_file_path, epi_file_path, configure_pool):
    ...
    for eachImage in trans_file_path:
        ...
        trans_image_list.append(
            crop_image(image_trans_tmp, cropXMin, cropXMax, cropYMin, cropYMax)
        )
    for eachImage in epi_file_path:
        image_epi_tmp = cv2.imread(eachImage)
        epi_image_list.append(
            crop_image(image_epi_tmp, cropXMin, cropXMax, cropYMin, cropYMax)
        )
    image_trans_calib = calculate_calib_background_gray(
        trans_image_list, gaussian_kernal, gaussian_iteration
    )
    (
        image_epi_calib_B,
        image_epi_calib_G,
        image_epi_calib_R,
    ) = calculate_calib_background_BGR(
        epi_image_list, gaussian_kernal, gaussian_iteration
    )
    np.savez(
        calib_parameter_PATH,
        image_trans_calib=image_trans_calib,
        image_epi_calib_B=image_epi_calib_B,
        image_epi_calib_G=image_epi_calib_G,
        image_epi_calib_R=image_epi_calib_R,
    )
...
def calculate_calib_background_BGR(image_list, gaussian_kernal, gaussian_iteration):
    ...
    return (
        image_epi_calib_B_mean_blur,
        image_epi_calib_G_mean_blur,
        image_epi_calib_R_mean_blur,
    )


def calculate_canny_upper(image_res_GB, size_subSample, percentile):
    image_res_GB_flatten = image_res_GB.flatten()
    image_res_GB_flatten_sub = np.random.choice(
        image_res_GB_flatten, size_subSample, replace=True
    )
    image_res_GB_flatten_sub_noZero = np.ma.masked_equal(
        image_res_GB_flatten_sub, 0
    ).compressed()
...
def calculate_background_GMM(image_gray_first_con, size_subSample, mean_empirical):
    ...
    GMM_input_data = np.random.choice(
        image_gray_first_flatten, size_subSample, replace=True
    )
    ...
    clf_Gray = GaussianMixture(
        n_components=2, max_iter=500, means_init=mean_init_empirical
    )
    clf_Gray.fit(GMM_input)
    index_Gray = [
        e
        for e in range(len(clf_Gray.means_))
        if clf_Gray.means_[e] == max(clf_Gray.means_)
    ][0]
...
def getFinalData(
    contours, configure_pool, image_trans_crop, image_epi_crop, height_crop, width_crop
):
    ...
    for contour in contours:
        ...
        image_trans_mask = cv2.bitwise_and(
            image_trans_crop[topY : (topY + height + 1), topX : (topX + width + 1)],
            image_trans_crop[topY : (topY + height + 1), topX : (topX + width + 1)],
            mask=blackImg[topY : (topY + height + 1), topX : (topX + width + 1)],
        )
        image_epi_mask = cv2.bitwise_and(
            image_epi_crop[topY : (topY + height + 1), topX : (topX + width + 1)],
            image_epi_crop[topY : (topY + height + 1), topX : (topX + width + 1)],
            mask=blackImg[topY : (topY + height + 1), topX : (topX + width + 1)],
        )
        gray_list = np.ma.masked_equal(image_trans_mask.flatten(), 0).compressed()
        Bepi_list = np.ma.masked_equal(
            image_epi_mask[:, :, 0].flatten(), 0
        ).compressed()
        Gepi_list = np.ma.masked_equal(
            image_epi_mask[:, :, 1].flatten(), 0
        ).compressed()
        Repi_list = np.ma.masked_equal(
            image_epi_mask[:, :, 2].flatten(), 0
        ).compressed()
...
def postprocess_segmentation(
    xStart, xEnd, yStart, yEnd, image_subSegmentation, image_subContour
):
    totalContours = []
...
def process_segmentation(image_subContour, image_subContour_binary, configure_pool):
    ...
    local_maxi = peak_local_max(
        distance,
        indices=False,
        footprint=np.ones((randowWalker_maxiSize, randowWalker_maxiSize)),
        labels=image_subContour_binary,
    )
...
def postprocess_contours(finalDF, finalContours, image_trans_crop, configure_pool):
    ...
    for i in range(len(finalDF)):
        ...
        modifiedContour, xStart, xEnd, yStart, yEnd = ZoomInContoursBox(
            contour, segment_bias + filter_bias
        )
        if (
            xStart < 0
            or yStart < 0
            or xEnd >= image_trans_crop.shape[1]
            or yEnd >= image_trans_crop.shape[0]
        ):
            continue
        ifPostProcessFlag = 0
        if (
            finalDF["Circularity"][i] < circularity_threshold
            and finalDF["Circularity"][i] >= circularity_threshold_veryBad
        ):
            if (
                finalDF["Area"][i] > area_segment_min
                and finalDF["Area"][i] < area_segment_max
            ):
                ifPostProcessFlag = 1
        ...
        if ifPostProcessFlag == 0:
            if (
                finalDF["Circularity"][i] > post_minCircularity
                and finalDF["Convexity"][i] > post_minConvexity
            ):
                post_finalContours.append(contour)
        else:
            modifiedContour, xStart, xEnd, yStart, yEnd = ZoomInContoursBox(
                contour, segment_bias
            )
            image_subContour_binary = np.zeros(
                (yEnd - yStart + 1, xEnd - xStart + 1), dtype=np.uint8
            )
            cv2.drawContours(image_subContour_binary, [modifiedContour], 0, 255, -1)
            cv2.drawContours(image_subContour_binary, [modifiedContour], 0, 255, 1)
            image_subContour = cv2.bitwise_and(
                image_trans_copy[yStart : (yEnd + 1), xStart : (xEnd + 1)].astype(
                    np.uint8
                ),
                image_trans_copy[yStart : (yEnd + 1), xStart : (xEnd + 1)].astype(
                    np.uint8
            ...
            image_subSegmentation = process_segmentation(
                image_subContour.copy(), image_subContour_binary.copy(), configure_pool
            )
            post_contours = postprocess_segmentation(
                xStart,
                xEnd,
                yStart,
                yEnd,
                image_subSegmentation,
                image_subContour.copy(),
            )
...
def generateMaskedSubImage(
    image_trans_copy, contour, colony_blank_offset, colony_contour_offset
):
    modifiedContour, xStart, xEnd, yStart, yEnd = ZoomInContoursBox(
        contour, colony_blank_offset
    )
    image_subContour_binary = np.zeros(
        (yEnd - yStart + 1, xEnd - xStart + 1), dtype=np.uint8
    )
    cv2.drawContours(image_subContour_binary, [modifiedContour], 0, 255, -1)
    cv2.drawContours(
        image_subContour_binary, [modifiedContour], 0, 255, colony_contour_offset
    )
    image_subContour = cv2.bitwise_and(
...
def plateQualityControl(
    ...
    while True:
        ...
        resize_image = cv2.resize(
            image_trans_crop,
            (int(width_crop * resize_factor), int(height_crop * resize_factor)),
        )
        ...
        but_test2 = Label(
            root,
            text="(total number of colonies on this plate: " + str(num_of_colony) + ")",
            font=("Times", textSizeSmall),
        )
        ...
        def click_Yes(varPool):
            ...
            confirm_test = Label(
                top, text="Keep this plate?", font=("Times", textSizeLarge)
            )
            confirm_test.grid(
                row=0, column=0, columnspan=5, sticky=W + E + S, padx=10, pady=10
            )
            ...
            confirm_button = Button(
                top,
                text="Yes",
                command=click_confirm_Yes_with_arg,
                foreground="green",
                font=("Times", textSizeButton),
            )
        ...
        def click_No(varPool):
            ...
            confirm_button = Button(
                top,
                text="No",
                command=click_confirm_No_with_arg,
                foreground="red",
                font=("Times", textSizeButton),
            )
            confirm_button.grid(row=1, column=2, padx=1, pady=1, sticky=W + E + N)
    ...
    while r_old != r:
        ...
        for i in range(n):
            ...
            cols_in_play = np.asarray(range(dist_mat.shape[1]))[np.newaxis, :][
                :, filter(lambda n: n not in no_i, range(dist_mat.shape[1]))
            ]
            mm = dist_mat[no_i, :][
                :, filter(lambda n: n not in no_i, range(dist_mat.shape[1]))
            ]
...
def pickColonyFirst(finalDF, num_of_pick, iteration):
    ...
    for robust_iter in range(iteration):
        ...
        print(
            "Finish farthest points iteration "
            + str(robust_iter)
            + "... (Execution time: "
            + str(time_dur)
            + ")"
...
def reSelectColony(num_of_pick, previous_pick, ignore_pick, post_finalDF_PCA):
    ...
    dist_mat = scipy.spatial.distance.cdist(
        post_finalDF_PCA, post_finalDF_PCA, metric="euclidean"
    )
    candidates = []
...
def generateContourSubImage_QC(
    image_trans_crop,
    contour,
    midpoint,
    segment_bias,
...
def colonyQualityControl(
    ...
    globalVar.image_picked_contours = drawContour(
        image_trans_crop, globalVar.pick_finalContours, colonyQC_colonyContourPixel
    )
    globalVar.image_picked_contours_label = drawContourLabel(
        globalVar.image_picked_contours,
        globalVar.pick_finalDF,
        range(1, len(globalVar.finalPick) + 1),
        colonyQC_colonyLabelSize,
        colonyQC_colonyLabelThickness,
    )
    ...
    while True:
        ...
        resize_image = cv2.resize(
            cv2image,
            (
                int(width_crop_merge * resize_factor * 18 / 19),
                int(height_crop_merge * resize_factor * 18 / 19),
            ),
        )
        ...
        for i in range(len(globalVar.finalPick)):
            tmpSub = generateContourSubImage_QC(
                image_trans_crop,
                globalVar.pick_finalContours[i],
                (globalVar.pick_finalDF["X"][i], globalVar.pick_finalDF["Y_concat"][i]),
                colony_show_bias,
                colony_show_size,
                1,
                str(i + 1),
                colony_show_size / 200.0,
                int(colony_show_size / 100.0),
            )
        ...
        def show_whole_image(varPool):
            frm_whole = Frame(
                root,
                width=str(int(width_crop * resize_factor)),
                height=str(int(height_crop * resize_factor)),
            )
            canvas_whole = Canvas(
                frm_whole,
                width=str(int(width_crop * resize_factor) - 30),
                height=str(int(height_crop * resize_factor)),
            )
            frm_image_whole = Frame(canvas_whole)
            myscrollbar_whole = Scrollbar(
                frm_whole, orient="vertical", command=canvas_whole.yview
            )
            ...
            panel.grid(
                row=0, column=0, columnspan=18, sticky=W + E + N + S, padx=5, pady=5
            )

            but_test = Label(
                root,
                text="View all selected colonies on plates",
                font=("Times", colonyQC_TextSizeLarge),
            )
            but_test.grid(
                row=1, rowspan=2, column=5, columnspan=9, sticky=W + E, padx=5, pady=10
            )
            ...
            but_continue = Button(
                root,
                text="Continue",
                command=click_continue_with_arg,
                foreground="black",
                font=("Times", colonyQC_TextSizeButton),
            )
        ...
        def show_ind_image(varPool):
            ...
            for i in range(len(varPool.finalPick)):
                ...
                if varPool.colony_button_flag[i] == 0:
                    tmpButton = Button(
                        frm_but,
                        command=button_fun_with_arg,
                        image=varPool.pick_colony_tk[i],
                    )
                else:
                    tmpButton = Button(
                        frm_but,
                        command=button_fun_with_arg,
                        image=varPool.pick_colony_bad_tk[i],
                    )
                tmpButton.grid(
                    row=i / 6, column=3 * (i % 6), columnspan=3, padx=7, pady=10
                )
            ...
            but_text1 = Label(
                root,
                text="Please select bad colonies",
                font=("Times", colonyQC_TextSizeMid),
            )
            but_text1.grid(
                row=1, column=5, columnspan=8, sticky=W + E + S, padx=5, pady=0
            )
            but_text2 = Label(
                root,
                text="(remaining colonies for re-select: "
                + str(varPool.remaining_colonies)
                + ")",
                font=("Times", colonyQC_TextSizeSmall),
            )
            but_text2.grid(
                row=2, column=5, columnspan=8, sticky=W + E + N, padx=5, pady=0
            )
            ...
            def click_reselect(varPool_1):
                tmpPick = [
                    varPool_1.finalPick[i]
                    for i in range(len(varPool_1.finalPick))
                    if varPool_1.colony_button_flag[i] == 0
                ]
                if len(tmpPick) == 0:
                    time.sleep(0.1)
                    top = Toplevel()
                    top.title("Warning")
                    top.geometry(
                        "x".join([str(confirm_size[0] + 60), str(confirm_size[1])])
                    )
                    warning_test = Label(
                        top,
                        text="At least one colony should be kept for re-pick",
                        font=("Times", colonyQC_TextSizeMid),
                    )
                    warning_test.grid(
                        row=0, column=0, columnspan=5, sticky=W + E + S, padx=5, pady=10
                    )
                    ...
                    warning_button = Button(
                        top,
                        text="OK",
                        command=click_warning_OK,
                        foreground="black",
                        font=("Times", colonyQC_TextSizeButton),
                    )
                    warning_button.grid(
                        row=1, column=2, padx=1, pady=1, sticky=W + E + N
                    )
                else:
                    toPick = min(
                        varPool_1.remaining_colonies,
                        varPool_1.num_of_pick - len(tmpPick),
                    )
                    varPool_1.remaining_colonies -= toPick
                    varPool_1.bad_colonies += [
                        varPool_1.finalPick[i]
                        for i in range(len(varPool_1.finalPick))
                        if varPool_1.colony_button_flag[i] == 1
                    ]
                    if toPick != 0:
                        pick_list = reSelectColony(
                            toPick, tmpPick, varPool_1.bad_colonies, post_finalDF_PCA
                        )
                        varPool_1.finalPick = tmpPick + pick_list
                        varPool_1.finalPick.sort()
                        varPool_1.pick_finalContours = [
                            post_finalContours[i] for i in varPool_1.finalPick
                        ]
                        varPool_1.pick_finalDF = post_finalDF.iloc[varPool_1.finalPick]
                        varPool_1.pick_finalDF.index = range(len(varPool_1.finalPick))
                        varPool_1.image_picked_contours = drawContour(
                            image_trans_crop,
                            varPool_1.pick_finalContours,
                            colonyQC_colonyContourPixel,
                        )
                        varPool_1.image_picked_contours_label = drawContourLabel(
                            varPool_1.image_picked_contours,
                            varPool_1.pick_finalDF,
                            range(1, len(varPool_1.finalPick) + 1),
                            colonyQC_colonyLabelSize,
                            colonyQC_colonyLabelThickness,
                        )
                        ...
                        for i in range(len(varPool_1.finalPick)):
                            tmpSub = generateContourSubImage_QC(
                                image_trans_crop,
                                varPool_1.pick_finalContours[i],
                                (
                                    varPool_1.pick_finalDF["X"][i],
                                    varPool_1.pick_finalDF["Y_concat"][i],
                                ),
                                colony_show_bias,
                                colony_show_size,
                                1,
                            ...
                            varPool_1.pick_colony_bad_tk.append(
                                image_to_tk(addRedXtoImage(tmpSub, 2))
                            )
                        ...
                        top.title("Warning")
                        top.geometry(
                            "x".join([str(confirm_size[0] + 15), str(confirm_size[1])])
                        )
                        warning_test = Label(
                            top,
                            text="There are no more colonies for re-pick",
                            font=("Times", colonyQC_TextSizeMid),
                        )
                        warning_test.grid(
                            row=0,
            ...
            def click_finish(varPool_1):
                ...
                confirm_test = Label(
                    top,
                    text="Are you sure to pick these colonies?",
                    font=("Times", colonyQC_TextSizeMid),
                )
                confirm_test.grid(
                    row=0, column=0, columnspan=5, sticky=W + E + S, padx=10, pady=10
                ...
                def click_confirm_Yes(varPool_2):
                    ...
                    top.destroy()
                    root.destroy()
                    varPool_2.outputPick = [
                        varPool_2.finalPick[i]
                        for i in range(len(varPool_2.finalPick))
                        if varPool_2.colony_button_flag[i] == 0
                    ]
                    varPool_2.breakFlag = 1
                    varPool_2.bad_colonies += [
                        varPool_2.finalPick[i]
                        for i in range(len(varPool_2.finalPick))
                        if varPool_2.colony_button_flag[i] == 1
                    ]
            ...
            but_re = Button(
                root,
                text="Re-pick",
                command=click_reselect_with_arg,
                foreground="red",
                font=("Times", colonyQC_TextSizeButton),
            )
            but_finish = Button(
                root,
                text="Finish",
                command=click_finish_with_arg,
                foreground="green",
                font=("Times", colonyQC_TextSizeButton),
            )
            but_back.grid(
                row=3, column=4, columnspan=3, sticky=W + E + N, padx=5, pady=5
            )
            but_re.grid(row=3, column=8, columnspan=3, sticky=W + E + N, padx=5, pady=5)
            but_finish.grid(
                row=3, column=12, columnspan=3, sticky=W + E + N, padx=5, pady=5
            )
...
def initializePlateInfo(globalObj):
    ...
    for i in range(totalImage):
        ...
        tmpMetadata["Y_concat"] = [
            0,
        ] * tmpMetadata.shape[0]
        tmpMetadata["pickStatus"] = [
            "not_pick",
        ] * tmpMetadata.shape[0]
        tmpMetadata["pickIndexGroup"] = [
            "NA",
        ] * tmpMetadata.shape[0]
        tmpMetadata["pickIndexPlate"] = [
            "NA",
        ] * tmpMetadata.shape[0]
...
def modifyOSconfigure(configurePool):
    if sys.platform == "darwin":
        configurePool["plateQC_colonyContourPixel"] = configurePool[
            "plateQC_macOS_colonyContourPixel"
        ]
        configurePool["plateQC_imageScaleFactor"] = configurePool[
            "plateQC_macOS_imageScaleFactor"
        ]
        configurePool["plateQC_imageWidthBias"] = configurePool[
            "plateQC_macOS_imageWidthBias"
        ]
        configurePool["plateQC_imageHeightBias"] = configurePool[
            "plateQC_macOS_imageHeightBias"
        ]
        configurePool["plateQC_ComfirmWindow"] = configurePool[
            "plateQC_macOS_ComfirmWindow"
        ]
        configurePool["plateQC_TextSizeLarge"] = configurePool[
            "plateQC_macOS_TextSizeLarge"
        ]
        configurePool["plateQC_TextSizeSmall"] = configurePool[
            "plateQC_macOS_TextSizeSmall"
        ]
        configurePool["plateQC_TextSizeButton"] = configurePool[
            "plateQC_macOS_TextSizeButton"
        ]
        configurePool["colonyQC_imageScaleFactor"] = configurePool[
            "colonyQC_macOS_imageScaleFactor"
        ]
        configurePool["colonyQC_imageWidthBias"] = configurePool[
            "colonyQC_macOS_imageWidthBias"
        ]
        configurePool["colonyQC_imageHeightBias"] = configurePool[
            "colonyQC_macOS_imageHeightBias"
        ]
        configurePool["colonyQC_ComfirmWindow"] = configurePool[
            "colonyQC_macOS_ComfirmWindow"
        ]
        configurePool["colonyQC_colonyShowBias"] = configurePool[
            "colonyQC_macOS_colonyShowBias"
        ]
        configurePool["colonyQC_colonyWindowBias"] = configurePool[
            "colonyQC_macOS_colonyWindowBias"
        ]
        configurePool["colonyQC_colonyContourPixel"] = configurePool[
            "colonyQC_macOS_colonyContourPixel"
        ]
        configurePool["colonyQC_colonyLabelSize"] = configurePool[
            "colonyQC_macOS_colonyLabelSize"
        ]
        configurePool["colonyQC_colonyLabelThickness"] = configurePool[
            "colonyQC_macOS_colonyLabelThickness"
        ]
        configurePool["colonyQC_TextSizeLarge"] = configurePool[
            "colonyQC_macOS_TextSizeLarge"
        ]
        configurePool["colonyQC_TextSizeMid"] = configurePool[
            "colonyQC_macOS_TextSizeMid"
        ]
        configurePool["colonyQC_TextSizeSmall"] = configurePool[
            "colonyQC_macOS_TextSizeSmall"
        ]
        configurePool["colonyQC_TextSizeButton"] = configurePool[
            "colonyQC_macOS_TextSizeButton"
        ]
    else:
        configurePool["plateQC_colonyContourPixel"] = configurePool[
            "plateQC_MSwin10_colonyContourPixel"
        ]
        configurePool["plateQC_imageScaleFactor"] = configurePool[
            "plateQC_MSwin10_imageScaleFactor"
        ]
        configurePool["plateQC_imageWidthBias"] = configurePool[
            "plateQC_MSwin10_imageWidthBias"
        ]
        configurePool["plateQC_imageHeightBias"] = configurePool[
            "plateQC_MSwin10_imageHeightBias"
        ]
        configurePool["plateQC_ComfirmWindow"] = configurePool[
            "plateQC_MSwin10_ComfirmWindow"
        ]
        configurePool["plateQC_TextSizeLarge"] = configurePool[
            "plateQC_MSwin10_TextSizeLarge"
        ]
        configurePool["plateQC_TextSizeSmall"] = configurePool[
            "plateQC_MSwin10_TextSizeSmall"
        ]
        configurePool["plateQC_TextSizeButton"] = configurePool[
            "plateQC_MSwin10_TextSizeButton"
        ]
        configurePool["colonyQC_imageScaleFactor"] = configurePool[
            "colonyQC_MSwin10_imageScaleFactor"
        ]
        configurePool["colonyQC_imageWidthBias"] = configurePool[
            "colonyQC_MSwin10_imageWidthBias"
        ]
        configurePool["colonyQC_imageHeightBias"] = configurePool[
            "colonyQC_MSwin10_imageHeightBias"
        ]
        configurePool["colonyQC_ComfirmWindow"] = configurePool[
            "colonyQC_MSwin10_ComfirmWindow"
        ]
        configurePool["colonyQC_colonyShowBias"] = configurePool[
            "colonyQC_MSwin10_colonyShowBias"
        ]
        configurePool["colonyQC_colonyWindowBias"] = configurePool[
            "colonyQC_MSwin10_colonyWindowBias"
        ]
        configurePool["colonyQC_colonyContourPixel"] = configurePool[
            "colonyQC_MSwin10_colonyContourPixel"
        ]
        configurePool["colonyQC_colonyLabelSize"] = configurePool[
            "colonyQC_MSwin10_colonyLabelSize"
        ]
        configurePool["colonyQC_colonyLabelThickness"] = configurePool[
            "colonyQC_MSwin10_colonyLabelThickness"
        ]
        configurePool["colonyQC_TextSizeLarge"] = configurePool[
            "colonyQC_MSwin10_TextSizeLarge"
        ]
        configurePool["colonyQC_TextSizeMid"] = configurePool[
            "colonyQC_MSwin10_TextSizeMid"
        ]
        configurePool["colonyQC_TextSizeSmall"] = configurePool[
            "colonyQC_MSwin10_TextSizeSmall"
        ]
        configurePool["colonyQC_TextSizeButton"] = configurePool[
            "colonyQC_MSwin10_TextSizeButton"
        ]


def drawOutputContoursWhole(
    image_trans,
    image_epi,
    all_contours,
    pick_index,
...
def saveOutputs_pickingOptimization(
    globalOutput, total_image, configure_pool, output_dir, groupID_index
):
    for i in range(total_image):
        ...
        f.close()
        output_image_trans, output_image_epi = drawOutputContoursWhole(
            tmp_image_trans,
            tmp_image_epi,
            tmp_all_contours,
            tmp_pick_index,
        ...
        cv2.imwrite(
            output_dir + "/" + tmp_image_label + "_Image_colony_trans_pick.jpg",
            output_image_trans,
        )
        cv2.imwrite(
            output_dir + "/" + tmp_image_label + "_Image_colony_epi_pick.jpg",
            output_image_epi,
        )
        output_image_trans_all = drawContour(tmp_image_trans, tmp_all_contours, 2)
        output_image_trans_all_pin = drawPinSite(
            output_image_trans_all, tmp_all_contours, 2
        )
        cv2.imwrite(
            output_dir + "/" + tmp_image_label + "_Image_colony_trans.jpg",
            output_image_trans_all_pin,
        )
        tmp_metadataDF.to_csv(output_dir + "/" + tmp_image_label + "_Metadata_all.csv")
        np.save(
            output_dir + "/" + tmp_image_label + "_Contours_all.npy", tmp_all_contours
        )
    timeStamp = time.time()
...
def modifyOutputObject_colonyDetection(globalOutput, total_image, configure_pool):
    for i in range(total_image):
        ...
        tmp_metadataDF["plate_barcode"] = [
            globalOutput.image_label[i],
        ] * len(tmp_all_contours)
...
def savePCAdata(
    globalOutput, configure_pool, sample_config_path, pick_dir, output_path
):
    ...
    for i in range(sample_config.shape[0]):
        globalOutput.groupID[
            image_labelIndex_pool[list(sample_config["barcode"])[i]]
        ] = list(sample_config["groupID"])[i]
    ...
    for eachGroupID in unique_groupID:
        ...
        groupID_image_trans_list = [
            globalOutput.image_trans_corrected[i] for i in groupID_index
        ]
        groupID_image_merge, groupID_image_heightStart = concatenateImages_gray(
            groupID_image_trans_list,
            groupID_label_list,
        ...
        newDF_dist = {
            "pickStatus": list(pick_csv["pickStatus"]),
            "plateBarcode": list(pick_csv["plate_barcode"]),
            "PCA1": list(preprocessed_plates[:, 0]),
            "PCA2": list(preprocessed_plates[:, 1]),
        }
